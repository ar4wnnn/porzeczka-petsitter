    ...
    <section className="py-24 bg-[var(--background-color)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <motion.h2 
            className="text-3xl md:text-4xl font-bold text-[var(--text-color)]"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            viewport={{ once: true }}
          >
            Co Mówią Nasi <span className="text-gradient-ocean">Klienci</span> 😺
          </motion.h2>
          <motion.p 
            className="mt-4 text-lg text-[var(--blue-medium)] max-w-2xl mx-auto"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className="relative max-w-5xl mx-auto">
              ...
              >
                <div className="md:w-1/3 flex-shrink-0 mb-6 md:mb-0">
                  <div className="relative w-32 h-32 md:w-48 md:h-48 mx-auto">
                    {testimonials[activeIndex].image ? (
                      <Image 
                        ...
                        fill
                        className="object-cover rounded-full border-4 border-[var(--blue-pale)]"
                      />
                    ) : (
                      <div className="w-full h-full rounded-full flex items-center justify-center bg-[var(--blue-accent)] text-4xl font-bold text-white">
                  ...
                  <div className="text-center mt-4">
                    ...
                    <p className="text-sm text-[var(--blue-medium)] mt-1">
                ...
                <div className="md:w-2/3 md:pl-8">
                  <svg className="h-10 w-10 text-[var(--blue-accent)] opacity-50 mb-4" fill="currentColor" viewBox="0 0 32 32">
                    <path d="M9.352 4C4.456 7.456 1 13.12 1 19.36c0 5.088 3.072 8.064 6.624 8.064 3.36 0 5.856-2.688 5.856-5.856 0-3.168-2.208-5.472-5.088-5.472-.576 0-1.344.096-1.536.192.48-3.264 3.552-7.104 6.624-9.024L9.352 4zm16.512 0c-4.8 3.456-8.256 9.12-8.256 15.36 0 5.088 3.072 8.064 6.624 8.064 3.264 0 5.856-2.688 5.856-5.856 0-3.168-2.304-5.472-5.184-5.472-.576 0-1.248.096-1.44.192.48-3.264 3.456-7.104 6.528-9.024L25.864 4z" />
                  </svg>
                  <p className="text-lg md:text-xl text-[var(--blue-dark)] italic mb-6">
                    "{testimonials[activeIndex].text}"
                  </p>
                  <div>
                    <p className="font-semibold text-lg text-[var(--blue-dark)]">{testimonials[activeIndex].name}</p>
                    <p className="text-[var(--blue-medium)]">{testimonials[activeIndex].location}</p>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
          ...
          <div className="flex justify-between mt-8">
            <button 
              onClick={handlePrev}
              className="bg-white hover:bg-gray-50 text-[var(--blue-medium)] p-3 rounded-full shadow-md transition-all hover:text-[var(--primary-color)]"
                ...
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-all ${
                    idx === activeIndex 
                      ...
                      : 'bg-gray-300 hover:bg-[var(--blue-pale)]'
                  }`}
